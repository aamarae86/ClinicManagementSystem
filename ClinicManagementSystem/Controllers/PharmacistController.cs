...
namespace ClinicManagementSystem.Controllers
{
    ...
    public class PharmacistController : Controller
    {
        ...
        {
            CallBackData callBackData = (new DoctorLogic()).Fetch(Request.FetchPaging(), "Pharmacist", Server.MapPath("~/Assets/images/ProfilePicture"));
        ...
        public ActionResult Create(AspNetUser model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (model.Id > 0)
                {
                    TempData["Message"] = new PharmacistLogic().SaveUpdate(model);
                ...
                else
                {
                    ...
                    if (result.Succeeded)
                    {
                        ...
                        TempData["Message"] = "Success : Pharmacist has been added successfully";
                        string profileName = user.FirstName + " " + (String.IsNullOrEmpty(user.LastName) ? string.Empty : user.LastName);
                        (new GenericModel()).FetchUserProfile().activityLog("Pharmacist creation", "<b>Profile of '" + profileName + "'</b> has been created", user.Id, "AspNetUser");
                        return Redirect("/Pharmacist/Index");
                    }
                    ...
                    (new GenericModel()).FetchUserProfile().activityLog("Pharmacist creation", "<b>Profile creation</b> has been failed", 0, "AspNetUser");
                    return View(model);
                }
        ...
        public ActionResult PatientMedicine()
        {
            return View();
        }

        public JsonResult FetchMedicine()
        {
            CallBackData callBackData = (new PatientLogic()).FetchMedicines(Request.FetchPaging());
            return Json(callBackData, JsonRequestBehavior.AllowGet);
        }

        [HttpPost]
        public ActionResult PatientMedicineHistory(int? id)
        {
            var model = new PatientLogic().GetPatientMedicineHistory(id.Value);
            return PartialView("_PatientMedicineHistory", model);
        }
    }
}