...
namespace ClinicManagementSystem.Controllers
{
    ...
    public class PatientController : Controller
    {
        ...
        {
            CallBackData callBackData = (new DoctorLogic()).Fetch(Request.FetchPaging(), "Patient", Server.MapPath("~/Assets/images/ProfilePicture"));
        ...
        public ActionResult Create(AspNetUser model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (model.Id > 0)
                {
                    TempData["Message"] = new PatientLogic().SaveUpdate(model);
                    return Redirect("/Patient/Index");
                }
                else
                {
                    var user = new ApplicationUser { UserName = model.UserName, Email = model.Email, Address = model.Address, FirstName = model.FirstName, LastName = model.LastName, CreatedTime = DateTime.Now, UpdatedTime = DateTime.Now, IsActive = model.IsActive, PhoneNumber = model.PhoneNumber, ClinicStartTime = model.ClinicStartTime, ClinicEndTime = model.ClinicEndTime, Gender = model.Gender, Qualification = model.Qualification, Experience = model.Experience, SpecializationId = model.SpecializationId, DateOfBirth = model.DateOfBirth, Photo = model.Photo,ProofOfResidence = model.ProofOfResidence,Disability = model.Disability,DisabilityDescription = model.DisabilityDescription };
                    var result = UserManager.Create(user, model.Password);
                    if (result.Succeeded)
                    {
                        UserManager.AddToRole(user.Id, "Patient");
                        TempData["Message"] = "Success : Patient has been added successfully";
                        string profileName = user.FirstName + " " + (String.IsNullOrEmpty(user.LastName) ? string.Empty : user.LastName);
                        (new GenericModel()).FetchUserProfile().activityLog("Patient creation", "<b>Profile of '" + profileName + "'</b> has been created", user.Id, "AspNetUser");
                        return Redirect("/Patient/Index");
                    }
                    ...
                    (new GenericModel()).FetchUserProfile().activityLog("Patient creation", "<b>Profile creation</b> has been failed", 0, "AspNetUser");
                    return View(model);
                }
