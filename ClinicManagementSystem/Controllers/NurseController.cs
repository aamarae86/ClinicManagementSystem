...
namespace ClinicManagementSystem.Controllers
{
    ...
    public class NurseController : Controller
    {
        ...
        {
            CallBackData callBackData = (new DoctorLogic()).Fetch(Request.FetchPaging(), "Doctor Assistance/Nurse", Server.MapPath("~/Assets/images/ProfilePicture"));
        ...
        public ActionResult Create(AspNetUser model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (model.Id > 0)
                {
                    TempData["Message"] = new NurseLogic().SaveUpdate(model);
                ...
                else
                {
                    ...
                    if (result.Succeeded)
                    {
                        UserManager.AddToRole(user.Id, "Doctor Assistance/Nurse");
                        TempData["Message"] = "Success : Nurse has been added successfully";
                        string profileName = user.FirstName + " " + (String.IsNullOrEmpty(user.LastName) ? string.Empty : user.LastName);
                        (new GenericModel()).FetchUserProfile().activityLog("Nurse creation", "<b>Profile of '" + profileName + "'</b> has been created", user.Id, "AspNetUser");
                        return Redirect("/Nurse/Index");
                    }
                    ...
                    (new GenericModel()).FetchUserProfile().activityLog("Nurse creation", "<b>Profile creation</b> has been failed", 0, "AspNetUser");
                    return View(model);
                }
