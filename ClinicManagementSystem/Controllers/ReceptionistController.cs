...
namespace ClinicManagementSystem.Controllers
{
    ...
    public class ReceptionistController : Controller
    {
        ...
        {
            CallBackData callBackData = (new DoctorLogic()).Fetch(Request.FetchPaging(), "Receptionist", Server.MapPath("~/Assets/images/ProfilePicture"));
        ...
        public ActionResult Create(AspNetUser model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (model.Id > 0)
                {
                    TempData["Message"] = new ReceptionistLogic().SaveUpdate(model);
                ...
                else
                {
                    ...
                    if (result.Succeeded)
                    {
                        ...
                        TempData["Message"] = "Success : Receptionist has been added successfully";
                        string profileName = user.FirstName + " " + (String.IsNullOrEmpty(user.LastName) ? string.Empty : user.LastName);
                        (new GenericModel()).FetchUserProfile().activityLog("Receptionist creation", "<b>Profile of '" + profileName + "'</b> has been created", user.Id, "AspNetUser");
                        return Redirect("/Receptionist/Index");
                    }
                    ...
                    (new GenericModel()).FetchUserProfile().activityLog("Receptionist creation", "<b>Profile creation</b> has been failed", 0, "AspNetUser");
                    return View(model);
                }
